...
class Tracker
  ...
  def initialize torrent, options = {}
    ...
    @options = {:tracker_timeout => 10, :use_announce_list_on_initial_connection => false, 
                :port => 6881}.merge(options)

    if @torrent_file.decoded_data['announce-list']
      @options[:use_announce_list_on_initial_connection] = true
    end
    ...
    decoded = @torrent_file.decoded_data

    info = decoded['info'].bencode
    ...
    @trackers = []
    torrent_hash = @torrent_file.decoded_data
